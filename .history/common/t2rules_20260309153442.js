export function applyT2Rules(data) {
  ...
  jsonData.forEach((row) => {
    ...
    // 记录初始状态
    log(
      `Init: Status='${orderStatus}', Fulfilment='${fulfillmentStatus}', RemarkLen=${fulfillmentRemark ? fulfillmentRemark.length : 0}`,
    );
    ...
    if (orderStatus === "Ready To Submit") {
      if (fulfillmentStatus === "In Progress") {
        action = orderType;
        log(
          `Rule 1.1: Status=Ready To Submit, Fulfilment=In Progress -> ${action}`,
        );
        if (createdBy !== "integration.user") {
          action = "Sales";
          log(
            `Rule 1.1.1: CreatedBy='${createdBy}' (!= integration.user) -> Sales`,
          );
        }
        ...
        if (createdBy !== "integration.user") {
          action = "Sales";
          log(
            `Rule 1.6.1: CreatedBy='${createdBy}' (!= integration.user) -> Sales`,
          );
        } else {
          action = orderType;
          log(
            `Rule 1.6.2: CreatedBy='${createdBy}' (= integration.user) -> ${orderType}`,
          );
        }
      } else if (
        ...
        action = orderType;
        log(
          `Rule 1.3: Status=Ready To Submit, Fulfilment='${fulfillmentStatus}' -> ${action}`,
        );
      } else if (fulfillmentStatus === "Inventory Replenishment") {
        action = "F&S";
        log(
          `Rule 1.4: Status=Ready To Submit, Fulfilment='${fulfillmentStatus}' -> ${action}`,
        );
      } else if (fulfillmentStatus === "Inventory Fallout") {
        action = "NORA";
        log(
          `Rule 1.5: Status=Ready To Submit, Fulfilment='${fulfillmentStatus}' -> ${action}`,
        );
      } else if (fulfillmentStatus === "Address Under Review") { 
        action = "Sales";
        log(
          `Rule 1.6: Status=Ready To Submit, Fulfilment='${fulfillmentStatus}' -> ${action}`,
        );
      } else {
        log(
          `Rule 1: Status=Ready To Submit, but Fulfilment='${fulfillmentStatus}' matched no sub-rule.`,
        );
      }
    } else if (orderStatus === "Amend Requested") {
      if (
        fulfillmentStatus === "Inventory Fallout" ||
        fulfillmentStatus === "Appointment Changed (M1)" ||
        ...
        if (createdBy !== "integration.user") {
          action = "N/A";
          log(
            `Rule 2.1.1: Status=Amend Requested, Fulfilment='${fulfillmentStatus}' -> ${action}`,
          );
        } else {
          action = orderType;
          log(
            `Rule 2.1.2: Status=Amend Requested, Fulfilment='${fulfillmentStatus}' -> ${action}`,
          );
        }
      } else {
        log(
          `Rule 2: Status=Amend Requested, but Fulfilment='${fulfillmentStatus}' matched no sub-rule.`,
        );
      }
      ...
      if (!fulfillmentStatus && !fulfillmentId) {
        action = orderType;
        log(
          `Rule 3.0: Status=In Progress, Fulfilment='${fulfillmentStatus}', Remark='${fulfillmentRemark ? "Yes" : "No"}' -> ${action}`,
        );
      } else if (
        !fulfillmentStatus ||
        ...
        action = orderType;
        log(
          `Rule 3.1: Status=In Progress, Fulfilment='${fulfillmentStatus}', Remark='${fulfillmentRemark ? "Yes" : "No"}' -> ${action}`,
        );
      } else if (fulfillmentStatus === "Appointment Changed (M2)") {
        action = "N/A";
        log(
          `Rule 3.2.1: Status=In Progress, Fulfilment=Appointment Changed (M2) -> N/A`,
        );
        ...
        action = "F&S";
        log(
          `Rule 3.3: Status=In Progress, Fulfilment=Inventory Replenishment -> F&S`,
        );
      } else if (fulfillmentStatus === "Address Check Sales Follow-up") {
        action = "Sales";
        log(
          `Rule 3.4: Status=In Progress, Fulfilment=Address Check Sales Follow-up -> Sales`,
        );
      } else if (fulfillmentStatus === "[F&S] Resources Issues") {
        action = "F&S";
        log(`Rule 3.5: Fulfilment='${fulfillmentStatus}' -> F&S`);
      } else if (fulfillmentStatus === "Address Check SB Assigned") {
        action = "Sales";
        log(`Rule 3.5: Fulfilment='${fulfillmentStatus}' -> Sales`);
      } else if(fulfillmentStatus === "Leased-In Failed"){
        action = "Sales";
        log(`Rule 3.6: Fulfilment='${fulfillmentStatus}' -> Sales`);
      } else if(fulfillmentStatus === "Waiting For Inventory"){
        if(fulfillmentRemark.includes("UIM")){
          action = "UIM";
          log(`Rule 3.7.1: INVENTORY FALLOUT + UIM -> UIM`);
        }else{
          action = "OPS";
          log(`Rule 3.7.2: Fulfilment='${fulfillmentStatus}' -> OPS`);
        }
      } else {
        log(
          `Rule 3: Status=In Progress, but Fulfilment='${fulfillmentStatus}' matched no sub-rule.`,
        );
      }
      ...
      if (fulfillmentStatus === "Completed") {
        action = "N/A";
        log(
          `Rule 3.3.1: Status=In Progress-Fulfilment Completed, Fulfilment= Completed -> N/A`,
        );
      }
    } else if (
      ...
      action = orderType;
      log(
        `Rule 4: Status='${orderStatus}', Fulfilment='${fulfillmentStatus}', ApptId='${appointmentId}' -> ${action}`,
      );
    } else if (
      fulfillmentStatus === "Address Approved" ||
      ...
      // 没有任何主要状态规则匹配
      log(
        `No Main Rule matched for Status='${orderStatus}', Fulfilment='${fulfillmentStatus}'`,
      );
    }
    ...
    if (action === null && typeof fulfillmentRemark === "string") {
      ...
      const remarks = fulfillmentRemark
        .split(/\n+(?=\[)/)
        .map((s) => s.trim())
        .filter((s) => s);
      ...
      const hasM1words = remarks.some((r) =>
        m1Criteria.some((k) => r.includes(k)),
      );
      ...
      if (remarks.length > 0) {
        ...
        if (firstRemark.includes("CANCELLED")) {
          ...
          action = "NORA";
          log(
            `Rule 6.4: First Remark includes NORA updated "ORDER ABORT" -> NORA`,
          );
          ...
          if (remarks.length > 1) {
            ...
            if (secondRemark.includes("2N Status: Rented")) {
              action = "N/A";
              log(
                `Rule 6.5.1: Second Remark includes 2N Status: Rented -> N/A`,
              );
            } else if (secondRemark.includes("2N Status: Failed")) {
              action = "Sales";
              log(
                `Rule 6.5.2: Second Remark includes 2N Status: Failed -> Sales`,
              );
        ...
        for (const remark of remarks) {
          if (remark.includes("APPOINTMENT CHANGED")) {
            if (hasM1words && orderStatus === "Amend Requested") {
              action = orderType;
              log(
                `Rule 7.1.1: APPOINTMENT CHANGED + M1 words + Amend Requested -> ${action}`,
              );
              break;
            }
            if (
              hasM1words &&
              (fulfillmentStatus === "Remake Appointment (M1)" ||
                fulfillmentStatus === "Remake Appointment (M2)")
            ) {
              action = "Sales";
              log(
                `Rule 7.1.2: APPOINTMENT CHANGED + M1 words + Remake Appointment -> Sales`,
              );
              break;
            }
            if (hasM1words) {
            ...
            if (remark.includes("Await Sales Follow Up")) {
              action = "Sales";
              log(
                `Rule 7.1.4: APPOINTMENT CHANGED + Await Sales Follow Up -> Sales`,
              );
              break;
            }
            if (
              (remark.includes("Customer Busy") ||
                remark.includes("await customer")) &&
              remark.includes("TID: true")
            ) {
              action = "N/A";
              log(
                `Rule 7.1.5: APPOINTMENT CHANGED + Customer Busy/await customer + TID: true -> N/A`,
              );
              break;
            }
            if (remark.includes("DRC: [21]") || remark.includes("DRC: [22]")) {
              action = "N/A";
              log(`Rule 7.1.6: APPOINTMENT CHANGED + DRC: [21]/[22] -> N/A`);
              break;
            }
            ...
            if (
              remark.includes("Customer Busy") &&
              remark.includes("TID: false")
            ) {
              action = "N/A";
              log(
                `Rule 7.1.10: APPOINTMENT CHANGED + Customer Busy + TID: false -> N/A`,
              );
              break;
            }
          }
          ...
          if (
            remark.includes("2N UPDATED") &&
            (remark.includes("2N Status: Failed") || remark.includes("SEE"))
          ) {
            action = "N/A";
          ...
          if (
            remark.includes("INVENTORY FALLOUT") ||
            remark.includes("Fallout Reason: Cable assignment issue")
          ) {
            ...
            if (remark.includes("UIM")) {
              ...
              if (remark.includes("MANUAL ASSIGNMENT REQUIRED") || remark.includes("NOT ALLOW AUTO ASSIGN FOR THIS SB")) {
                action = "BAND";
                log(
                  `Rule 7.3.3.1: INVENTORY FALLOUT + UIM + MANUAL ASSIGNMENT REQUIRED -> BAND`,
                );
              }
              break;
            }
          ...
          if(remark.includes("Fallout Reason: NOSS related issue (VNDP)")){
            action = "VNDP";
            log(`Rule 7.6: NOSS related issue (VNDP) -> VNDP`);
            break;
          }

          if(remark.includes("<head><title>504 Gateway Time-out</title></head>")){
            action = "NORA";
            log(`Rule 7.7: 504 Gateway Time-out -> NORA`);
            break;
          }
