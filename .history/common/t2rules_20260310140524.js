export function applyT2Rules(data) {
  ...
  jsonData.forEach((row) => {
    ...
    // 2. 根据状态评估规则
    let action = evaluateStatusRules(ctx);
...
function evaluateStatusRules(ctx) {
  const { status, fulfillStatus, fulfillRemark, fulfillId, apptId, createdBy, orderType, log } = ctx;
  ...
  if (status === "Amend Requested") {
    ...
    if(fulfillStatus === "In Progress" || fulfillStatus === "In Progress-Distributed"){
      log(`[Status Rule 2.3] Amend Requested + '${fulfillStatus}' -> N/A`);
      return "orderType";
    }
  } 
    ...
    if (fulfillStatus === "Waiting For Inventory") {
      if (fulfillRemark.includes("UIM")) {
        if(remark.includes("MANUAL ASSIGNMENT REQUIRED") || 
          remark.includes("NOT ALLOW AUTO ASSIGN FOR THIS SB") ||
          remark.includes("NO AVAILABLE DP")){
...
function applySalesOverride(currentAction, ctx, m1Criteria) {
  ...
  const {createdBy, status, fulfillStatus, fulfillRemark, log } = ctx;
  ...
  if (finalAction === "Sales" && 
    ...
    if ( paragraphs.length > 0) {
      ...
      if (hasM1words) {
        ...
        if(status === "Ready To Submit" && createdBy != "integration.user"){
          finalAction = "N/A";
