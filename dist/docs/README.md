...
- **用户友好的界面**
  - **功能中心**首页采用手机 App 图标式布局：所有功能按 连接与环境 / LTS / PCD / 数据分析 / 工具 等分组展示，点击图标进入对应功能，左上角「返回」或 `Esc` 回到首页。
  - 首页顶部有 **常用功能** 栏，**顺序完全由你自己决定，不会随使用频率变化**：把鼠标移到任意功能卡片上点右下角的 `☆` 加入 / 移出，点常用功能区右上角「编辑」后用 `←` `→` 调序、`×` 移出，右上角「恢复默认」可一键还原。改动保存在本地，跟随浏览器账号持久化。
  - 图标排列、分组、文案、配色均可通过右上角 **设置 → 布局配置** 里的 JSON 自定义（改完即生效，无需重新打包）。
  - 实时状态通知和进度提示。
...
### 4. 设置与自定义首页布局（设置 → 布局配置）

点击界面 **右上角的「设置」**（齿轮按钮），可以看到设置页的各项：
...
│   └── ui_layout_default.js # 首页布局内置默认值（兜底）
├── common/            # 通用工具模块
